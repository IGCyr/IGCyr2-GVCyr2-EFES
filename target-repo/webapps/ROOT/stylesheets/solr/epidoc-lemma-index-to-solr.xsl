...
<xsl:stylesheet exclude-result-prefixes="#all"
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="lemmata" select="distinct-values(tokenize(normalize-space($lemma-values), '\s+'))" />
    <add>
      <xsl:for-each select="$lemmata">
        ...
        <doc>
          ...
          <field name="index_item_name">
            <xsl:if test="not($w/@lemma) and $w/@part=('F','M')"><xsl:text>-</xsl:text></xsl:if>
            <xsl:value-of select="normalize-unicode(.,'NFD')" />
            <xsl:if test="not($w/@lemma) and $w/@part=('I','M')"><xsl:text>-</xsl:text></xsl:if>
