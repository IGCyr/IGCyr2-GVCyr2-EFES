...
<xsl:stylesheet exclude-result-prefixes="#all"
  ...
  <xsl:template match="/">
    <add>
      <xsl:for-each-group select="//tei:measure[@type='currency']" group-by="concat(normalize-unicode(normalize-space(string-join(., ''))), '-', @unit)">
        <doc>
          ...
          <field name="index_item_type">
            <xsl:choose>
              <xsl:when test="@unit">
                <xsl:value-of select="replace(replace(@unit, 'sdrachma', 's drachma'), 'smna', 's mna')"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:text>?</xsl:text>
              </xsl:otherwise>
